...
func Init(customConf string) error {
	var err error
	File, err = ini.LoadSources(ini.LoadOptions{IgnoreInlineComment: true}, "custom/conf/app.ini")
	if err != nil {
	...
	if util.IsFile(customConf) {
		...
		log.Warnf("Custom config %q not found. Ignore this warning if you're running for the first time\n\n", customConf)
	}
